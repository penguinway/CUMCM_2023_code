...
def inputs(points):
    df = pd.read_excel(io="point12.xlsx", sheet_name="Sheet1", header=0)
    for i in range(0, num):
        point_ij = Point(x=-(df.values[i][1] / (sqrt(df.values[i][1] ** 2 + df.values[i][2] ** 2 + H ** 2))),
                         y=-(df.values[i][2] / (sqrt(df.values[i][1] ** 2 + df.values[i][2] ** 2 + H ** 2))),
                         z=H / (sqrt(df.values[i][1] ** 2 + df.values[i][2] ** 2 + H ** 2)))
...
result = {"eta_at": [], "eta_ref": [], "eta_turnc": [], "eta_cos": [], "power": []}
for i in range(0, num):
    ...
    dh = sqrt(use_point.x ** 2 + use_point.y ** 2 + use_point.z ** 2)
    eta_at = 0.99321 - 0.0001176 * dh + 1.97E-8 * dh ** 2
    result["eta_at"].append(eta_at)
    result["eta_ref"].append(0.92)
    result["eta_turnc"].append(cal_turnc(a, b, dh))
    result["eta_cos"].append(sum_yita / 60)
    result["power"].append(sum_power / 60)
    sum_yita = 0
print(sum_power / 60)
print(sum_power / (60*a*b*num))
df1 = pd.DataFrame(result)
file_name = "result_121.xlsx"
